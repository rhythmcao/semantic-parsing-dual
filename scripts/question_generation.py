...
from utils.solver.solver_question_generation import QGSolver
...
qg_vocab = Vocab(opt.dataset, task='question_generation', copy=opt.copy)
...
if not opt.testing:
    params = {
        ...
        "src_vocab": len(qg_vocab.lf2id), "tgt_vocab": len(qg_vocab.word2id),
        "src_unk_idx": qg_vocab.lf2id[UNK], "tgt_unk_idx": qg_vocab.word2id[UNK],
        "pad_src_idxs": [qg_vocab.lf2id[PAD]], "pad_tgt_idxs": [qg_vocab.word2id[PAD]],
...
if not opt.testing:
    ratio1 = load_embeddings(train_model.src_embed.embed, qg_vocab.lf2id, opt.device)
    ratio2 = load_embeddings(train_model.tgt_embed.embed, qg_vocab.word2id, opt.device)
    logger.info("%.2f%% token embeddings from pretrained vectors" % (ratio1 * 100))
    logger.info("%.2f%% word embeddings from pretrained vectors" % (ratio2 * 100))
else:
...
loss_function = set_loss_function(ignore_index=qg_vocab.word2id[PAD], reduction=opt.reduction)
optimizer = set_optimizer(train_model, lr=opt.lr, l2=opt.l2, max_norm=opt.max_norm)

###################################### Training and Decoding #######################################

solver = QGSolver(train_model, qg_vocab, loss_function, optimizer, exp_path, logger, device=opt.device)
if not opt.testing:
    ...
    bleu = solver.decode(test_dataset, os.path.join(exp_path, 'test.eval'),
        opt.test_batchSize, beam=opt.beam, n_best=opt.n_best)
    logger.info('Evaluation cost: %.4fs\tAcc : %.4f' % (time.time() - start_time, bleu))